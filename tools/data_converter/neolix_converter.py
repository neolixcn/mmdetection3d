...
from .neolix_data_utils import get_neolix_image_info
...
def create_neolix_info_file(data_path,
                           pkl_prefix='neolix',
                           save_path=None,
                           relative_path=True):
    """Create info file of Neolix dataset.
    ...
    neolix_infos_train = get_neolix_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, neolix_infos_train, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Neolix info train file is saved to {filename}')
    mmcv.dump(neolix_infos_train, filename)
    neolix_infos_val = get_neolix_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
        ...
        relative_path=relative_path)
    _calculate_num_points_in_gt(data_path, neolix_infos_val, relative_path)
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'Neolix info val file is saved to {filename}')
    mmcv.dump(neolix_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Neolix info trainval file is saved to {filename}')
    mmcv.dump(neolix_infos_train + neolix_infos_val, filename)

    neolix_infos_test = get_neolix_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    print(f'Neolix info test file is saved to {filename}')
    mmcv.dump(neolix_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    """
    neolix_infos = mmcv.load(info_path)

    for info in mmcv.track_iter_progress(neolix_infos):
        pc_info = info['point_cloud']
        # image_info = info['image']
        # calib = info['calib']

...
def neolix_eval(gt_annos,
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            ...
            if mAP3d is not None:
                ret_dict[f'Neolix/Overall_3D_{postfix}'] = mAP3d[idx, 0]
            if mAPbev is not None:
                ret_dict[f'Neolix/Overall_BEV_{postfix}'] = mAPbev[idx, 0]
            if mAPbbox is not None:
                ret_dict[f'Neolix/Overall_2D_{postfix}'] = mAPbbox[idx, 0]

    return result, ret_dict


def neolix_eval_coco_style(gt_annos, dt_annos, current_classes):
